...
static char *get_next_field(char *p, char *id, char *value)
{
	...
	for (n = 0; is_id_char(*p) && n < FIELD_LENGTH; n++)
		id[n] = *(p++);
	if (n >= FIELD_LENGTH)
		error("Error: field too long in 'use description'.");
	id[n] = '\0';
	if (*(p++) != ':')
		error("Error: missing ':' in 'use description'.");
	...
	while (isspace(*p))
		++p;		/* skip spaces */
	for (n = 0; *p != '#' && *p != '\0' && n < FIELD_LENGTH; n++)
		value[n] = *(p++);
	if (n >= FIELD_LENGTH)
		error("Error: field too long in 'use description'.");
	value[n] = '\0';
...
static char *get_next_token(char *p, char div, char *value)
{
	...
	while (isspace(*p))
		++p;		/* skip spaces */
	for (n = 0; *p != div && *p != '\0' && n < FIELD_LENGTH; n++)
		value[n] = *(p++);
	if (n >= FIELD_LENGTH)
		error("Error: field too long in 'use description'.");
	value[n] = '\0';
...
static void process_new_argument(char *id, char *value, struct arguments *arg)
{
	...
	for (i = 0; i < arg->arg_num; i++)
		if (strcmp(arg->args[i].name, arg->args[arg->arg_num].name) ==
		    0)
			error
	...
	if (arg->args[arg->arg_num].required
	    && arg->args[arg->arg_num].def_value)
		error
...
static void process_argument_description(char *desc, struct arguments *arg)
{
	...
	if (desc == NULL || arg == NULL)
		error
		    ("Error: invalid input to 'process_argument_description'.");
...
static void use(struct arguments *arg)
{
	...
	if (arg == NULL)
		error("Error: invalid input to 'use'.");
	...
	fprintf(stderr, "  %s\tPrint this help message and exit.\n",
		HELP_OPTION);
	fprintf(stderr,
		"  %s\tPrint version and compilation date/time and exit.\n",
		VERSION_OPTION);
	for (i = 0; i < arg->arg_num; i++)
		if (!(arg->args[i].required)) {
			fprintf(stderr, "  -%c", arg->args[i].id);
			...
			if (arg->args[i].type == 'i') {
				...
				if (arg->args[i].def_value)
					fprintf(stderr, ", default value %d",
						atoi(arg->args[i].d_value));
				fprintf(stderr, "\n");
			}
			if (arg->args[i].type == 'd') {
				fprintf(stderr, "\t\t'%s' is double",
					arg->args[i].name);
				fprintf(stderr, ", range [");
				if (arg->args[i].min_set)
					fprintf(stderr, "%g,",
...
static void evaluate_arguments(int argc, char **argv, struct arguments *arg)
{
	...
	/* check input */
	if (argc <= 0)
		error("Error: unexpected command line: missing command.");
	...
	for (n = 1; !in_required_args && n < argc; n++) {
		...
		if (strlen(argv[n]) != 2) {
			...
			if (strcmp(argv[n], HELP_OPTION) == 0)
				use(arg);
...
static struct arguments *process_arguments(char *desc, int argc, char **argv)
{
	...
	for (i = 0; i < arg->arg_num; i++)
		if (arg->args[i].required && !(arg->args[i].assigned))
			use(arg);
	...
	for (i = 0; i < arg->arg_num; i++)
		if (strcmp(name, arg->args[i].name) == 0) {
			if (arg->args[i].type == 's') {
				...
				error
				    ("Error: get_str: the parameter is not a double.");
		}
	...
	for (i = 0; i < arg->arg_num; i++)
		if (strcmp(name, arg->args[i].name) == 0) {
			...
			if (arg->args[i].type == 'i')
				...
				error
				    ("Error: get_int: the parameter is not an integer.");
		}
	...
	for (i = 0; i < arg->arg_num; i++)
		if (strcmp(name, arg->args[i].name) == 0) {
			...
			if (arg->args[i].type == 'd')
				...
				error
				    ("Error: get_double: the parameter is not a double.");
		}
		      ...
		      char *filename, int xsize, int ysize, double width)
{
	...
	for (i = 0; i < n; i++) {
		fprintf(eps,
			"newpath %f %f moveto %f %f lineto %f setlinewidth stroke\n",
			segs[i * dim + 0], (double)ysize - segs[i * dim + 1],
			segs[i * dim + 2], (double)ysize - segs[i * dim + 3],
...
int main(int argc, char **argv)
{
	...
	if (output == NULL)
		error("Error: unable to open ASCII output file.");
	for (i = 0; i < n; i++) {
		for (j = 0; j < dim; j++)
